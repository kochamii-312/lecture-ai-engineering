...
def main():
    ...
    with tab1:
        ...
        st.sidebar.markdown(
            "講義アンケートのexcelファイルをアップロードすると、重要なコメントや危険度の高いコメントを分析します。  \n"
            "センチメント分類：ポジティブ・ネガティブコメントをクラスタリングした代表コメントと割合  \n"
            "カテゴリ分類：4つのカテゴリに分類したコメントをクラスタリングした代表コメントと割合  \n"
            "重要度スコア：具体性・緊急性・共通性を総合的に評価した重要度スコアが高いコメントと分布"
        )
